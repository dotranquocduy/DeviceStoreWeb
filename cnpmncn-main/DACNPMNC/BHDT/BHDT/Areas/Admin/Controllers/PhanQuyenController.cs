...
namespace BHDT.Areas.Admin.Controllers
{
    ...
    public class PhanQuyenController : Controller
    {
        ...
        {
            return View(db.LoaiTKs.OrderBy(n=>n.MaLoaiTK));
        }
        [HttpGet]
        public ActionResult PhanQuyen(int? id)
        {
            ...
            LoaiTK lkh = db.LoaiTKs.SingleOrDefault(n => n.MaLoaiTK == id);
            if (lkh == null)
            {
                return HttpNotFound();
            }
            ...
            ViewBag.LoaiTKQuyen = db.LoaiQuyenTKs.Where(n=>n.MaLoaiTK==id);
            return View(lkh);
        }
        [HttpPost]
        public ActionResult PhanQuyen(int? id, IEnumerable<LoaiQuyenTK> lstLTKQuyen)
        {
            ...
            var lstDaPhanQuyen = db.LoaiQuyenTKs.Where(n => n.MaLoaiTK == id);
            if (lstDaPhanQuyen.Count()>=0 )
            {
                db.LoaiQuyenTKs.RemoveRange(lstDaPhanQuyen);
                db.SaveChanges();
            }
            if (lstLTKQuyen != null) { 
